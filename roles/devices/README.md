...
This role is intended for one-time bootstrapping of raw disks for use with IBM Informix server instances and is responsible for creating multipath daemon configuration, adding udev rules to binding character devices to block devices and create symbolic links for use in Informix configuration.

:warning: There is a **risk of data loss** when using this role. See [Provisioning hosts with existing data](#provisioning-hosts-with-existing-data) for more information if this role needs to be applied to hosts that contain existing data.
